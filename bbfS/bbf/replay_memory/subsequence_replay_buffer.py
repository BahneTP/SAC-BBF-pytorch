...
@gin.configurable
class SubsequenceParallelEnvReplayBuffer(object):
  ...
  """
    ...
    """
        ...
        t_indices = self._rng.integers(
            min_id, max_id, size=batch_size, dtype=np.int64
        ) % self._replay_length
        b_indices = self._rng.integers(
            0, self._n_envs, size=batch_size, dtype=np.int64
        )
        ...
        for i in range(len(t_indices)):
            ...
            if not is_valid:
                ...
                while not is_valid and allowed_attempts > 0:
                    ...
                    # is not stratified.
                    t_index = self._rng.integers(
                        min_id, max_id, size=1, dtype=np.int64
                    ) % self._replay_length
                    b_index = self._rng.integers(
                        0, self._n_envs, size=1, dtype=np.int64
                    )
                    allowed_attempts -= 1
                    t_indices[i] = t_index
                    b_indices[i] = b_index
    ...
    Args:
      rng: NumPy RNG, if overriding the default buffer state.
      batch_size: int, number of transitions returned. If None, the default
        batch_size will be used.
    ...
    """
        ...
        update_horizons = np.ones(batch_size * subseq_len,
                                  dtype=np.int32) * (update_horizon - 1)
...
@gin.configurable
class PrioritizedSubsequenceParallelEnvReplayBuffer(
        SubsequenceParallelEnvReplayBuffer):
        ...
        for i in range(len(indices)):
            ...
            if not is_valid:
                ...
                while (not is_valid) and allowed_attempts > 0:
                    # If index i is not valid keep sampling others. Note that this
                    # is not stratified.
                    index = int(self.sum_tree.stratified_sample(1, rng=self._rng))
    ...
    def get_transition_elements(self, batch_size=None, subseq_len=None):
        ...
        return parent_transition_type + probabilities_type
